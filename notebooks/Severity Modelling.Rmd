...
modelling_df <- severity_df %>% select("claim_paid", 
                                       "pet_gender", 
                                       "pet_de_sexed_age", 
                                       "pet_is_switcher", 
                                       "pet_age_months", 
...
train_df <- na.omit(modelling_df[modelling_df$claim_paid>0,])
...
```{r}
model <- train(claim_paid ~ . -earned_units + offset(log_earned_units), 
                               data = train_df, 
                               method = "glm", 
