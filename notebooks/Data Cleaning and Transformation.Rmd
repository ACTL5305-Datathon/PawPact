...
earned_data <- earned_data %>%
  mutate(earned_units = ifelse(earned_units == 0, 1, earned_units))
...
#  mutate(claim_day_diff = as.numeric(claim_start_date - UW_Date) + 1,
#         manual_earned_units = claim_day_diff / days_in_month(UW_Date),
#         earned_units = coalesce(na_if(earned_units, 0), manual_earned_units)) %>%
#  select(-claim_day_diff, -manual_earned_units)
...
```{r}
severity_df %>% filter(earned_units == 0)
```
